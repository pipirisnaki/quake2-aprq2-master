	...
	PVOID UserContext)
{
	...
	if (GetFileVersionInfo(ModuleName, 0, sizeof(buffer), buffer)) {
		if (VerQueryValue(buffer, "\\", (LPVOID*)&data, &numBytes)) {
	...
	if (pSymGetModuleInfo(processHandle, ModuleBase, &moduleInfo)) {
		// ModuleName = moduleInfo.ModuleName; // This would overwrite the const pointer
...
#define GPA( x, y )															\
	...
	fprintf(crashReport, "Address: %08x (%s)\n",
		(unsigned int)context->Eip, moduleName);

	fprintf(crashReport, "\nThread context:\n");
	fprintf(crashReport, "EIP: %08x EBP: %08x ESP: %08x\n",
		(unsigned int)context->Eip, (unsigned int)context->Ebp, (unsigned int)context->Esp);
	fprintf(crashReport, "EAX: %08x EBX: %08x ECX: %08x\n",
		(unsigned int)context->Eax, (unsigned int)context->Ebx, (unsigned int)context->Ecx);
	fprintf(crashReport, "EDX: %08x ESI: %08x EDI: %08x\n",
		(unsigned int)context->Edx, (unsigned int)context->Esi, (unsigned int)context->Edi);
		...
		NULL))
	{
		fprintf(crashReport, "%d: %08x %08x %08x %08x ",
			count,
			(unsigned int)stackFrame.Params[0],
			(unsigned int)stackFrame.Params[1],
			(unsigned int)stackFrame.Params[2],
			(unsigned int)stackFrame.Params[3]);
